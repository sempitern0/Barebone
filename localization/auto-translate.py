...
def create_template_pot_file(translation_keys: dict):
    with open(
        os.path.join(CURRENT_DIR, translation_template_name), "w+", encoding="utf8"
    ) as file:
        file.write(
            """# Barebone-Template - Translation template:
#
...
msgid ""
msgstr ""
"Project-Id-Version: Barebone-Template\\n"
"MIME-Version: 1.0\\n"
...
def create_or_update_translation_files(selected_locales=[]):
    translation_template_path = (
        os.path.join(CURRENT_DIR, translation_template_name).lstrip().replace("\\", "/")
    )

    for locale in selected_locales:
        current_po_translation_path = (
            os.path.join(CURRENT_DIR, f"{locale}.po").lstrip().replace("\\", "/")
        )
        ...
        if os.path.exists(current_po_translation_path):
            ...
            subprocess.run(
                [
                    "msginit",
                    "--no-translator",
                    f"--input={translation_template_path}",
                    f"--locale={current_po_translation_path}",
                ],
                check=True,
                capture_output=True,
                text=True,
...
def included_files_filter(file: str) -> bool:
    included_extensions = [".tscn", ".scn", ".tres", ".res", ".gd", ".cs"]
            ...
            for translation_key in translation_keys_from(line):
                if not _contains_godot_prefixes(translation_key):
                    translation_keys[translation_key] = (
                        os.path.relpath(filepath, GODOT_ROOT_DIR)
                        .lstrip()
                        .replace("\\", "/")
                    )
