...
func _fill_available_resolutions() -> void:
	...
	for display_resolution in OmniKitWindowManager.resolutions:
		if _resolution_is_included(display_resolution):
			add_separator(display_resolution)
			
			var allowed_resolutions =  OmniKitWindowManager.resolutions[display_resolution].filter(func(screen_size): return screen_size <= OmniKitHardwareDetector.computer_screen_size) if use_computer_screen_resolution_limit else OmniKitWindowManager.resolutions[display_resolution] 
...
func resolutions_based_on_hardware() -> void:
	if (OmniKitHardwareDetector.is_steam_deck()):
		display16_10 = true
		display16_9 = false
		display21_9 = false
		display4_3 = false
	elif OmniKitHardwareDetector.is_mobile():
		display16_10 = false
		display16_9 = false
		display21_9 = false
		display4_3 = false
...
func _resolution_is_included(resolution: String) -> bool:
	return resolution == OmniKitWindowManager.Resolution4_3 && display4_3 \
		or resolution == OmniKitWindowManager.Resolution16_9 && display16_9 \
		or resolution == OmniKitWindowManager.Resolution16_10 && display16_10 \
		or resolution == OmniKitWindowManager.Resolution21_9 && display21_9 \
		or resolution == OmniKitWindowManager.Resolution_Mobile && display_mobile
