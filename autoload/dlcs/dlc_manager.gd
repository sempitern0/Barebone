...
func load_dlcs(base_directory: String) -> void:
	...
	if error != OK:
		OmnikitLogger.error("DLCManager: The regex to find dlc files raised an error %d %s" % [error, error_string(error)])
		return
	...
	for dlc_path: String in dlc_file_paths:
		...
		if loaded:
			OmnikitLogger.info("DLCManager: The dlc %s has been loaded succesfully!" % dlc_path)
			...
			OmnikitLogger.error("DLCManager: The dlc %s could not be loaded" % dlc_path)
