...
signal started_turn_session(ordered_sockets: Array[TurnitySocket])
...
func start_new_serial_turn_session(ordered_sockets: Array[TurnitySocket], turn_duration: float = 0, max_turns: int = 0) -> Error:
	...
	started_turn_session.emit(current_turnity_sockets)
...
func get_sockets_from_node(node: Node) -> Array[TurnitySocket]:
	var sockets: Array[TurnitySocket] = []
	sockets.assign(find_nodes_of_custom_class(node, TurnitySocket))
...
## Example NodePositioner.find_nodes_of_custom_class(self, TurnitySocket)
