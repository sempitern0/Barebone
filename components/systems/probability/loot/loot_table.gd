...
func chance(random_number_generator: RandomNumberGenerator, probability_chance: float = 0.5, less_than: bool = true) -> bool:
	probability_chance = clamp(probability_chance, 0.0, 1.0)
	
	return random_number_generator.randf() < probability_chance if less_than else random_number_generator.randf() > probability_chance
