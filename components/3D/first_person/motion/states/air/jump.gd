...
func physics_update(delta: float):
	...
	if OmniKitInputHelper.action_just_pressed_and_exists(InputControls.JumpAction) and jump_count < jump_times:
		apply_jump()
		
	if shorten_jump_on_input_release and OmniKitInputHelper.action_just_released_and_exists(InputControls.JumpAction):
		shorten_jump()
	
	if (not actor.was_grounded and actor.is_grounded) or actor.is_on_floor():
...
func get_gravity() -> float:
	var up_direction_opposite: Vector3 = OmniKitVectorHelper.up_direction_opposite_vector3(actor.up_direction)
...
func detect_fall_after_jump() -> void:
	var up_direction_opposite: Vector3 = OmniKitVectorHelper.up_direction_opposite_vector3(actor.up_direction)
