class_name SlideState extends GroundState
...
@export_range(0, 360.0, 1.0, "radians_as_degrees") var slide_tilt: float = deg_to_rad(5.0)
...
func enter():
	...
	crouch_animation(actor.head_crouch_height, 0.2)
...
func exit(next_state: MachineState) -> void:
	...
	if not next_state is CrouchState:
		crouch_animation(actor.head_stand_height, 0.2)
...
func physics_update(delta):
	...
	if snappedf(actor.get_ground_speed(), 0.01) < 0.1:
		end_slide()
		return
	
	if swing_head and slide_tilt > 0:
		actor.camera_controller.camera.rotation.z =  lerp_angle(actor.camera_controller.camera.rotation.z, slide_side * slide_tilt, delta * slide_lerp_speed)
   
	if not actor.ceil_detector.is_colliding():
		detect_jump()
...
func end_slide() -> void:
	...
	if actor.crouch and actor.ceil_detector.is_colliding():
...
func _create_slide_timer() -> void:
	slide_timer = OmniKitTimeHelper.create_physics_timer(slide_time, false, true)
