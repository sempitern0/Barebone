...
var target_stair_step_y: float = 0.0


func enter() -> void:
	stair_stepping_enabled = actor.stairs


func exit(next: MachineState) -> void:
	stair_stepping_enabled = actor.stairs and next is GroundState
...
func detect_run() -> void:
	if actor.run and OmniKitInputHelper.action_pressed_and_exists(InputControls.RunAction):
		FSM.change_state_to(RunState)


func detect_crouch() -> void:
	if actor.crouch and OmniKitInputHelper.action_pressed_and_exists(InputControls.CrouchAction):
		FSM.change_state_to(CrouchState)
...
func detect_slide() -> void:
	if actor.crouch and actor.slide and OmniKitInputHelper.action_pressed_and_exists(InputControls.CrouchAction):
		FSM.change_state_to(SlideState)
...
func crouch_animation(target_height: float, duration: float) -> void:
	crouch_tween = create_tween()
	crouch_tween.tween_property(actor.head, "position:y", target_height, duration)\
...
func stair_step_up():
	...
	# 6. Move player up
	actor.global_position.y = test_transform.origin.y
...
func stair_step_down():
	...
	if actor.velocity.y <= 0 and actor.was_grounded:
		...
		if PhysicsServer3D.body_test_motion(actor.get_rid(), body_test_params, body_test_result):
			...
			actor.global_position.y += body_test_result.get_travel().y
