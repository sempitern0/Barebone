## This class needs to be added on Globals in the Project
extends Node

## A set of surfaces groups that points to a scene that display a decal when hitscan or bullet collide
@export var surface_decals: Dictionary[StringName, PackedScene] = {}
## A set of surfaces groups that points to a scene that display an impact particle when collide
@export var impacts: Dictionary[StringName, PackedScene] = {}


func _ready() -> void:
	GlobalEvents.weapon_fired.connect(on_weapon_fired)
...
func spawn_impact_on(collider: Node3D, collision_point: Vector3, normal: Vector3) -> void:
	...
	for collider_surface_group: StringName in collider_groups:
		if collider_surface_group in surface_decals.keys():
			var surface_decal: OmniKitSmartDecal = surface_decals[collider_surface_group].instantiate() as OmniKitSmartDecal
...
func spawn_impact_from_hitscan(hitscan: OmniKitRaycastResult) -> void:
	if hitscan.collided():
...
func on_weapon_fired(weapon: FireArmWeapon, hitscan: OmniKitRaycastResult) -> void:
	if weapon.configuration.hitscan:
