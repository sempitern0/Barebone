@icon("res://components/3D/dungeon_crawler/motion/dungeon_movement.svg")
...
func _can_move(direction: Vector2) -> bool:
	return target and _tween_can_run() and not direction.is_zero_approx() and direction in OmniKitVectorHelper.directions_v2


func _can_rotate(direction: Vector2) -> bool:
	return target and _tween_can_run() and direction in OmniKitVectorHelper.horizontal_directions_v2
