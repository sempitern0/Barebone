...
func push_bodies_on_range():
	for body: RigidBody3D in get_overlapping_bodies()\
		...
		body.angular_velocity = OmniKitVectorHelper.generate_3d_random_fixed_direction() * rng.randf_range(0.5, 1.0)
		body.apply_impulse(OmniKitVectorHelper.rotate_horizontal_random(direction) * rng.randf_range(min_push_force, max_push_force), -body.position.normalized() + upward_offset)
...
func _create_alive_timer():
	if alive_timer == null:
		alive_timer = OmniKitTimeHelper.create_physics_timer(max(0.05, time_alive), false, true)
