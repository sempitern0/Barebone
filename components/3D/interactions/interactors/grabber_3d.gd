@icon("res://components/3D/interactions/interactors/icons/grabber.svg")
...
@export var enabled: bool = true:
	set(value):
		...
		set_physics_process(enabled and active_grabbables.size() > 0)
...
func _input(_event: InputEvent) -> void:
	if push_wave_ability \
		...
		and OmniKitInputHelper.action_just_pressed_and_exists(InputControls.PushWave):
			
		push_wave()
		
	if pull_individual_ability and OmniKitInputHelper.action_just_pressed_and_exists(InputControls.Pull):
		if slots_available():
	...
	if pull_area_ability and \
		OmniKitInputHelper.action_just_pressed_and_exists(InputControls.PullArea) \
			 and slots_available():
	...
	if OmniKitInputHelper.action_just_pressed_and_exists(InputControls.Drop):
		for active_grabbable: ActiveGrabbable in active_grabbables:
			drop_body(active_grabbable.body)
			
	elif OmniKitInputHelper.action_just_pressed_and_exists(InputControls.Throw):
		for active_grabbable: ActiveGrabbable in active_grabbables:
...
func push_wave():
	if push_wave_ability:
		var wave = PushWaveArea3D.new(OmniKitCamera3DHelper.forward_direction(get_viewport().get_camera_3d()))
		add_child(wave)
		wave.activate()
	...
	if grabbable_area_detector and grabbable_area_detector.monitoring:
		...
		bodies.sort_custom(
			func(a: Grabbable3D, b: Grabbable3D): 
				return OmniKitNodePositioner.global_distance_to_v3(a, self) <= OmniKitNodePositioner.global_distance_to_v3(b, self))
