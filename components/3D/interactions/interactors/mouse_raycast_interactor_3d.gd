...
func _unhandled_input(event: InputEvent) -> void:
	if is_processing() and current_camera is Camera3D:
		...
		if interact_mouse_button == MOUSE_BUTTON_LEFT and OmniKitInputHelper.is_mouse_left_click(event) \
			or interact_mouse_button == MOUSE_BUTTON_RIGHT and OmniKitInputHelper.is_mouse_right_click(event):
				interact(current_interactable)
	
	if is_processing() and OmniKitInputHelper.action_just_pressed_and_exists(InputControls.CancelInteraction) \
		and current_interactable is Interactable3D:
...
func get_detected_interactable():
	...
	var ray_query = PhysicsRayQueryParameters3D.create(
		from, 
		to,
		Globals.world_collision_layer | Globals.interactables_collision_layer
	...
	if OmniKitInputHelper.is_mouse_visible() and result.has("collider") and result["collider"] is Interactable3D:
...
func interact(interactable: Interactable3D):
	if interactable and interactable.can_be_interacted:
		...
		GlobalEvents.interactable_interacted.emit(interactable)
