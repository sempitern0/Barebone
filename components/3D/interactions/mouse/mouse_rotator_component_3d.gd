@icon("res://components/3D/interactions/mouse/rotator.svg")
...
@export var target: Node3D:
	set(new_target):
		if target != new_target:
			...
			if target:
				original_target_rotation = target.rotation
			else:
				original_target_rotation = Vector3.ZERO
...
@export var use_rotation_z: bool = false
...
var original_target_rotation: Vector3 = Vector3.ZERO
	...
	if target:
		if event is InputEventMouseMotion and mouse_button_holded(event):
			...
			if use_rotation_x:
				...
				target.rotation.x = original_target_rotation.x
				
			if use_rotation_y:
				target.rotate_y(motion.relative.x * mouse_sens)
			else:
				target.rotation.y = original_target_rotation.y
				
			if use_rotation_z:
				target.rotate_z(motion.relative.y * mouse_sens)
			else:
				target.rotation.z = original_target_rotation.z
...
func mouse_button_holded(event: InputEvent) -> bool:
	...
	return (mouse_rotation_button == MOUSE_BUTTON_LEFT and OmniKitInputHelper.is_mouse_left_button_pressed(event)) \
			or (mouse_rotation_button == MOUSE_BUTTON_RIGHT and OmniKitInputHelper.is_mouse_right_button_pressed(event))
			
			
func mouse_release_detected(event: InputEvent) -> bool:
	return (mouse_rotation_button == MOUSE_BUTTON_LEFT and not OmniKitInputHelper.is_mouse_left_button_pressed(event)) \
			or (mouse_rotation_button == MOUSE_BUTTON_RIGHT and not OmniKitInputHelper.is_mouse_right_button_pressed(event))
...
func reset_target_rotation() -> void:
	if target:
		if reset_rotation_on_release and not target.rotation.is_equal_approx(original_target_rotation) \
			and (reset_rotation_tween == null or (reset_rotation_tween and not reset_rotation_tween.is_running())):
				...
				reset_rotation_tween.tween_property(target, "rotation", original_target_rotation, 0.5).from(target.rotation)\
