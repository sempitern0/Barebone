...
@icon("res://components/3D/interactions/interactables/icons/grabbable.svg")
class_name Grabbable3D extends RigidBody3D

const MaximumTransparency: int = 255
const GroupName: StringName = &"grabbables"
...
@export_range(0, 255, 1) var transparency_value_on_pull: int = MaximumTransparency:
	set(value):
		transparency_value_on_pull = clampi(value, 0, MaximumTransparency)
...
func _enter_tree() -> void:
	collision_layer = Globals.grabbables_collision_layer
	collision_mask = Globals.world_collision_layer | Globals.grabbables_collision_layer
...
func _ready() -> void:
	if mesh_instance == null:
		mesh_instance = OmniKitNodeTraversal.first_node_of_type(self, MeshInstance3D.new())
...
func throw() -> void:
	if state_is_pull():
		...
		var impulse: Vector3 = OmniKitCamera3DHelper.forward_direction(get_viewport().get_camera_3d()) * throw_power
...
func drop() -> void:
	if state_is_pull():
		...
		var impulse: Vector3 = OmniKitCamera3DHelper.forward_direction(get_viewport().get_camera_3d()) * Vector3.ZERO
		apply_impulse(impulse)
