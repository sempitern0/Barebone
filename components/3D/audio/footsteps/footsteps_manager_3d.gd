@icon("res://components/3D/audio/footsteps/footsteps.svg")
...
@export var sound_queue: AcousticSoundQueue
...
func _ready() -> void:
	if floor_detector_raycast == null:
		floor_detector_raycast = OmniKitNodeTraversal.first_node_of_type(self, RayCast3D.new())
		
	if sound_queue == null:
		sound_queue = OmniKitNodeTraversal.first_node_of_custom_class(self, AcousticSoundQueue)
		
	assert(floor_detector_raycast is RayCast3D, "FoostepManager: This node needs a raycast to detect the ground material")
	assert(sound_queue is AcousticSoundQueue, "FoostepManager: This node needs a AcousticSoundQueue to play the footstep sounds")
...
func _create_interval_timer():
	if interval_timer == null:
		interval_timer = OmniKitTimeHelper.create_physics_timer(default_interval_time, false, true)
