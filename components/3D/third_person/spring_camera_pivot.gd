...
@export var max_spring_length_zoom_out: float = 6.0
## The maximum spring length when zooming in camera
@export var max_spring_length_zoom_in: float = 1.0
@export var zoom_out_step: float = 0.5
@export var zoom_in_step: float = 0.5
## Smoothness for the spring arm limits in the world
...
func _physics_process(delta: float) -> void:
	if not mouse_capture.mouse_input.is_zero_approx():
		rotation.x += mouse_capture.pitch_input
		rotation.x = clampf(rotation.x, min_vertical_angle, max_vertical_angle)

		rotation.y += mouse_capture.twist_input
