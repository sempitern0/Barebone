class_name ThirdPersonSpringArmPivot extends Node3D
...
func _unhandled_input(event: InputEvent) -> void:
	if press_action_to_manual_camera and OmniKitInputHelper.action_pressed_and_exists(press_action_to_manual_camera):
		camera_manual_motion = true
	elif press_action_to_manual_camera.is_empty() and OmniKitInputHelper.is_mouse_captured():
		camera_manual_motion = true
	else:
	...
	if OmniKitInputHelper.action_pressed_and_exists(InputControls.ZoomInCamera):
		camera_zoom_in()
		
	elif OmniKitInputHelper.action_pressed_and_exists(InputControls.ZoomOutCamera):
		camera_zoom_out()
