...
func create_dungeon_rooms() -> void:
	for room: DungeonRoom in OmniKitArrayHelper.flatten(dungeon_grid)\
...
func entrance_room() -> DungeonRoom:
	...
	return OmniKitArrayHelper.flatten(dungeon_grid).filter(
...
func inside_tree_rooms() -> Array[DungeonRoom]:
	...
	rooms.assign(OmniKitArrayHelper\
...
func _toggle_ceils() -> void:
	for room: DungeonRoom in OmniKitArrayHelper\
		.flatten(dungeon_grid)\
...
func _generate_final_mesh() -> void:
	...
	if rooms.size():
		...
		OmniKitNodeTraversal.set_owner_to_edited_scene_root(final_mesh_root_node)
