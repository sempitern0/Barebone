...
func build() -> void:
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(self)
...
func create_materials_on_room() -> void:
	if configuration.generate_materials:
		var shapes =  OmniKitNodeTraversal.get_all_children(self).filter(func(child): return child is CSGShape3D)
		var index: int = 0
...
func walls() -> Array[CSGShape3D]:
	...
	for wall: CSGShape3D in OmniKitArrayHelper.remove_falsy_values([front_wall, back_wall, right_wall, left_wall]):
		result.append(wall)
...
func door_sockets() ->  Array[Marker3D]:
	var markers = OmniKitNodeTraversal.find_nodes_of_type(self, Marker3D.new())
	var sockets: Array[Marker3D] = []
	...
	if wall.get_child_count() == 0:
		...
		if configuration.randomize_door_position_in_wall:
			if door_rotation.y != 0 and (wall.size.z - _door_size.x) > _door_size.x:
				door_position.z = (-1 if OmniKitMathHelper.chance(0.5) else 1) * randf_range(_door_size.x, (wall.size.z - _door_size.x) / 2)
			
			if door_rotation.y == 0 and (wall.size.x - _door_size.x) > _door_size.x:
				door_position.x = (-1 if OmniKitMathHelper.chance(0.5) else 1) * randf_range(_door_size.x, (wall.size.x - _door_size.x) / 2)
		...
		OmniKitNodeTraversal.set_owner_to_edited_scene_root(door_slot)
		...
		OmniKitNodeTraversal.set_owner_to_edited_scene_root(room_socket)
...
func create_ceil_columns(size: Vector3 = configuration.room_size) -> void:
	...
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(ceil_column_base)
	ceil_column_base.add_child(ceil_column_substraction)
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(ceil_column_substraction)
...
func create_corner_columns(size: Vector3 = configuration.room_size) -> void:
	...
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(top_right_column)
	...
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(top_left_column)

	bottom_right_column.name = "BottomRightCornerColumn"
	bottom_right_column.size = column_size
	add_child(bottom_right_column)
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(bottom_right_column)
	...
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(bottom_left_column)
...
func create_floor(size: Vector3 = configuration.room_size) -> void:
	...
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(floor_side)
...
func create_ceil(size: Vector3 = configuration.room_size) -> void:
	...
	add_child(ceil_side)
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(ceil_side)
...
func create_front_wall(size: Vector3 = configuration.room_size) -> void:
	...
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(front_wall)
...
func create_back_wall(size: Vector3 = configuration.room_size) -> void:
	...
	add_child(back_wall)
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(back_wall)
...
func create_right_wall(size: Vector3 = configuration.room_size) -> void:
	...
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(right_wall)
...
func create_left_wall(size: Vector3 = configuration.room_size) -> void:
	...
	add_child(left_wall)
	OmniKitNodeTraversal.set_owner_to_edited_scene_root(left_wall)
