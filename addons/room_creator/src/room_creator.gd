...
func save_generated_meshes() -> void:
	...
	for room: RoomMesh in rooms_created:
		...
		for child: Node in OmniKitNodeTraversal.get_all_children(target_room):
		...
		match type_of_collision:
			AvailableCollisions.Convex:
				...
				if create_static_body:
					...
					OmniKitNodeTraversal.set_owner_to_edited_scene_root(body)
					body.add_child(convex_collision)
				else:
					room_mesh_instance.add_child(convex_collision)
					
				OmniKitNodeTraversal.set_owner_to_edited_scene_root(convex_collision)
			...
			AvailableCollisions.Trimesh:
				...
				OmniKitNodeTraversal.set_owner_to_edited_scene_root(trimesh_collision)
	...
	if _tool_can_be_used():
		...
		if generate_mesh_mode == MeshGenerationMode.MeshPerRoom:
			if room_meshes_output_node == null:
				...
				OmniKitNodeTraversal.set_owner_to_edited_scene_root(room_meshes_output_node)
			...
			for room: CSGRoom in new_rooms:
				...
				if room_mesh_instance:
					...
					OmniKitNodeTraversal.set_owner_to_edited_scene_root(room_mesh_instance)
			...
			if meshes.size() > 1:
				...
				for socket: Marker3D in OmniKitNodeTraversal.find_nodes_of_type(csg_combiner_root, Marker3D.new()):
...
func add_door_sockets_to_generated_room_mesh(room_mesh_instance: RoomMesh) -> void:
	for socket: Marker3D in room_mesh_instance.sockets:
		...
		OmniKitNodeTraversal.set_owner_to_edited_scene_root(door_socket)
...
func _prepare_csg_rooms_output_node() -> Node3D:
	if csg_rooms_output_node == null:
		...
		OmniKitNodeTraversal.set_owner_to_edited_scene_root(csg_rooms_output_node)
...
func _prepare_csg_combiner_root() -> CSGCombiner3D:
	...
	if not csg_combiner_root.is_inside_tree():
		...
		OmniKitNodeTraversal.set_owner_to_edited_scene_root(csg_combiner_root)
