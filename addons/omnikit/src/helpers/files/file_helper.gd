...
static func copy_directory_recursive(from_dir :String, to_dir :String) -> Error:
	if not DirAccess.dir_exists_absolute(from_dir):
		push_error("OmniKitFileHelper->copy_directory_recursive: directory not found '%s'" % from_dir)
	...
	if source_dir != null:
		...
		push_error("OmniKitFileHelper->copy_directory_recursive: Directory not found: " + from_dir)
		return ERR_DOES_NOT_EXIST
