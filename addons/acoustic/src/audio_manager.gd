...
#region Master Bus Audio Effects
...
func get_actual_volume_db_from_bus_name(bus_name: String) -> float:
	...
	if bus_index == -1:
		push_error("AcousticAudioManager: Cannot retrieve volume for bus name %s, it does not exists" %  bus_name)
		return 0.0
...
func get_bus(bus) -> int:
	...
	if typeof(bus_index) == TYPE_STRING or typeof(bus_index) == TYPE_STRING_NAME:
		...
		if bus_index == -1:
			push_error("AcousticAudioManager:get_bus() -> The bus with the name %s does not exists in this project" % bus)
...
func apply_music_bos_low_pass_filter() -> void:
	AudioServer.set_bus_effect_enabled(AcousticAudioManager.music_bus(), MusicBusLowPassFilterEffect, true)


func remove_music_bos_low_pass_filter() -> void:
	AudioServer.set_bus_effect_enabled(AcousticAudioManager.music_bus(), MusicBusLowPassFilterEffect, false)
