...
func _ready():
	var pool_amount := ProjectSettings.get_setting(AcousticAudioSettings.SoundPoolAmountSetting)
...
func play(stream: AudioStream, bus: StringName = AcousticAudioManager.MasterBus):
...
func play_with_pitch(stream: AudioStream, pitch_scale: float = 1.0, bus: StringName = AcousticAudioManager.MasterBus):
	if _bus_is_valid(bus):
		var available_stream_player = _next_available_stream_player()
...
func play_with_pitch_range(stream: AudioStream, min_pitch_scale: float = 0.9, max_pitch_scale: float = 1.3, bus: StringName = AcousticAudioManager.MasterBus):
	if _bus_is_valid(bus):
		var available_stream_player = _next_available_stream_player()
		
		if available_stream_player:
			play_with_pitch(stream, randf_range(min_pitch_scale, max_pitch_scale), bus)


func play_random_stream(streams: Array[AudioStream] = [], bus: StringName = AcousticAudioManager.MasterBus):
	if streams.is_empty() or not _bus_is_valid(bus):
		return
		
	play(streams.pick_random(), bus)


func play_random_stream_with_pitch(streams: Array[AudioStream] = [], pitch_scale: float = 1.0, bus: StringName = AcousticAudioManager.MasterBus):
	if streams.is_empty() or not _bus_is_valid(bus):
		return
		
	play_with_pitch(streams.pick_random(), pitch_scale, bus)


func play_random_stream_with_pitch_range(streams: Array[AudioStream] = [], min_pitch_scale: float = 0.9, max_pitch_scale: float = 1.3,  bus: StringName = AcousticAudioManager.MasterBus):
	if streams.is_empty() or not _bus_is_valid(bus):
		return
		
	play_with_pitch_range(streams.pick_random(), min_pitch_scale, max_pitch_scale, bus)


func stop_streams_from_bus(bus: StringName = AcousticAudioManager.MasterBus):
	for player: AudioStreamPlayer in stream_players_pool:
		if player.bus.to_lower() == bus.to_lower():
			player.stop()


func stop_streams_from_buses(buses: Array[String] = [AcousticAudioManager.MasterBus]):
	for bus in buses:
		stop_streams_from_bus(bus)


func pause_streams_from_bus(bus: StringName = AcousticAudioManager.MasterBus):
	for player: AudioStreamPlayer in stream_players_pool:
		if player.bus.to_lower() == bus.to_lower():
			player.stream_paused = true


func pause_streams_from_buses(buses: Array[String] = [AcousticAudioManager.MasterBus]):
	for bus in buses:
		pause_streams_from_bus(bus)


func unpause_streams_from_bus(bus: StringName = AcousticAudioManager.MasterBus):
	for player: AudioStreamPlayer in stream_players_pool:
		if player.bus.to_lower() == bus.to_lower():
			player.stream_paused = false


func unpause_streams_from_buses(buses: Array[String] = [AcousticAudioManager.MasterBus]):
	for bus in buses:
