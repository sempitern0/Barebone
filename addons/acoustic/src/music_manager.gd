...
var music_bank: Dictionary[StringName, MusicTrack] = {}
var music_playlists: Dictionary[StringName, MusicPlaylist] = {}

#region AudioStreamPlayers
## Two audio streams players are used to apply the crossfade between tracks
...
func _ready():
	if get_tree().root.has_node(AcousticAudioSettings.AudioManagerSingleton):
		_create_audio_stream_players(get_tree().root.get_node(AcousticAudioSettings.AudioManagerSingleton).MusicBus)
...
func start_playlist(playlist, from_track: int = 0, crossfade: bool = true, crossfade_time: float = default_crossfade_time) -> void:
	...
	if typeof(playlist) == TYPE_STRING or typeof(playlist) == TYPE_STRING_NAME:
		next_playlist = music_playlists.get(playlist, null)
		
		if next_playlist == null:
			push_error("AcousticMusicManager->start_playlist: The playlist with the provided name %s does not exist in the music playlist bank" % playlist)
			return
	elif playlist is MusicPlaylist:
		next_playlist = playlist
	else:
		push_error("AcousticMusicManager->start_playlist: The playlist provided is not valid, supported types are STRING & MusicPlaylist resource")
		return
		
	if next_playlist.tracks.is_empty():
		push_error("AcousticMusicManager->start_playlist: The playlist %s is empty" % next_playlist.playlist_name)
		return
		
	from_track = clampi(from_track, 0, next_playlist.tracks.size() - 1)
	...
	if current_playlist.is_random_mode():
		play_music_track(next_playlist.tracks.pick_random(), crossfade, crossfade_time)
		
	elif current_playlist.is_lineal_mode():
		play_music_track(next_playlist.tracks[from_track], crossfade, crossfade_time)
...
func play_music_from_bank(track, crossfade: bool = true, crossfade_time: float = default_crossfade_time):
	...
	if typeof(track) == TYPE_STRING or typeof(track) == TYPE_STRING_NAME:
		...
		if next_track == null:
			push_error("AcousticMusicManager->play_music_from_bank: Expected music name %s to exists in the MusicBank but no track was found" % track)
			return
	
	elif track is MusicTrack:
		next_track = track

	play_music_track(next_track, crossfade, crossfade_time)


func get_music_from_bank(track_name: StringName) -> MusicTrack:
	return music_bank.get(track_name, null)


func play_music_track(track: MusicTrack, crossfade: bool = true, crossfade_time: float = default_crossfade_time):
	if track.stream == null:
		push_error("AcousticMusicManager->play_music_track: The music track %s does not have an audio stream" % track.track_name)
		return
		
	if _crossfade_tween_is_running():
		push_warning("AcousticMusicManager->play_music_track: The crossfade tween is running, aborting playing the track %s" % track.track_name)
		return
	...
	if current_audio_stream_player.is_playing() and crossfade:
		...
		next_audio_stream_player.play(track.from_position)
		...
		current_audio_stream_player.play(track.from_position)
		current_track = track
...
func pause_current_track() -> void:
	if current_audio_stream_player.stream and not current_audio_stream_player.stream_paused:
		current_audio_stream_player.stream_paused = true


func replay_current_track() -> void:
	if current_audio_stream_player.stream and current_audio_stream_player.stream_paused:
		current_audio_stream_player.play()
	

#region Music bank
func pick_random_track_from_bank(except: Array[MusicTrack]= []) -> MusicTrack:
	var tracks: Array[MusicTrack] = music_bank.values()\
		.filter(func(track: MusicTrack): return not track in except)
...
func play_next_in_playlist(playlist: MusicPlaylist = current_playlist) -> void:
	if playlist:
		...
		if playlist.loop:
			play_music_track(next_in_playlist(playlist))
...
func next_in_playlist(playlist: MusicPlaylist = current_playlist) -> MusicTrack:
	...
	if playlist_tracks.size() > 0:
		if playlist.is_random_mode():
			if current_track and playlist_tracks.size() > 1:
				return playlist_tracks.filter(func(track: MusicTrack): return track != current_track)\
					.pick_random()
			
			return playlist_tracks.pick_random()
			
		elif playlist.is_lineal_mode():
			if current_track:
...
func _create_audio_stream_players(bus: StringName):
	main_audio_stream_player = _create_music_audio_stream_player("AcousticMainAudioStreamPlayer", bus)
	secondary_audio_stream_player = _create_music_audio_stream_player("AcousticSecondaryAudioStreamPlayer", bus)
